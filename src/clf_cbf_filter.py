...
class CBFFilter():
    ...
    def lie_derivatives(self, v_lead: float, x: np.ndarray) -> tuple[float, float]:
        ...
        lg_h = -self.tau / self.dynamics.m
        lf_V = (-2 * (x[1] - self.v_des) * self.dynamics.drag_force(x[1])) / self.dynamics.m
        lg_V = (2 * (x[1] - self.v_des)) / self.dynamics.m
        return float(lf_h), float(lg_h), float(lf_V), float(lg_V)

    def filter_control(
            self, x: np.ndarray, v_lead : float) -> tuple[float, float]:
        h_val = self.h(x)
        lf_h, lg_h, lf_V, lg_V = self.lie_derivatives(v_lead, x)
        ...
        constraints = [
            lf_h + lg_h * u >= -self.gamma * h_val,
            lf_V + lg_V * u <= -self.clf_rate * V_val + delta,
            u >= self.u_min,
            u <= self.u_max,
            delta >= 0.0,
        ]
